import { buildXiaomiProvider, XIAOMI_DEFAULT_MODEL_ID, buildZhipuProvider, ZHIPU_DEFAULT_MODEL_ID } from "../agents/models-config.providers.js";
import {
...
export function applyZhipuProviderConfig(cfg: OpenClawConfig): OpenClawConfig {
  ...
  models[ZHIPU_DEFAULT_MODEL_REF] = {
    ...models[ZHIPU_DEFAULT_MODEL_REF],
    alias: models[ZHIPU_DEFAULT_MODEL_REF]?.alias ?? "Zhipu",
  };

  const providers = { ...cfg.models?.providers };
  const existingProvider = providers.zhipu;
  const defaultProvider = buildZhipuProvider();
  const existingModels = Array.isArray(existingProvider?.models) ? existingProvider.models : [];
  const defaultModels = defaultProvider.models ?? [];
